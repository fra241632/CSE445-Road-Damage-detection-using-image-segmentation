"""
EDA / sanity-check utilities for the crack detection dataset:
- checks image/mask pairing
- reports image resolutions
- reports crack (foreground) vs background pixel ratio
"""
import os
import glob
import cv2
...
def report_stats(pairs):
    for img_path, mask_path in pairs[:5]:
        ...
        print(f"{os.path.basename(img_path)}: img={img.shape}, "
              f"mask={mask.shape}, crack_pixels={fg_ratio:.3%}")

if __name__ == "__main__":
    print("Point this at data/crack/images and data/crack/masks once downloaded.")
